...
bool OpenArm_v10HW::parse_config(const hardware_interface::HardwareInfo& info) {
  ...
  // Parse gravity compensation params
  ...
  gravity_urdf_path_ =
      (it != info.hardware_parameters.end()) ? it->second : "";
  ...
  RCLCPP_INFO(rclcpp::get_logger("OpenArm_v10HW"),
              ...
              "gravity_comp=%s, gravity_scale=%.3f",
...
hardware_interface::CallbackReturn OpenArm_v10HW::on_init(
  ...
  if (gravity_compensation_) {
    if (gravity_urdf_path_.empty() || gravity_base_link_.empty() ||
        gravity_tip_link_.empty()) {
      RCLCPP_ERROR(rclcpp::get_logger("OpenArm_v10HW"),
                   "gravity_compensation=true but urdf/base/tip link not set");
      return CallbackReturn::ERROR;
    }
    dynamics_ = std::make_unique<Dynamics>(gravity_urdf_path_,
                                           gravity_base_link_,
                                           gravity_tip_link_);
    if (!dynamics_->Init()) {
      RCLCPP_ERROR(rclcpp::get_logger("OpenArm_v10HW"),
...
hardware_interface::return_type OpenArm_v10HW::write(
  ...
  // Control arm motors with MIT control (with gravity feed-forward)
  std::vector<openarm::damiao_motor::MITParam> arm_params;
  for (size_t i = 0; i < ARM_DOF; ++i) {
    const double ff_tau =
        tau_commands_[i] + gravity_scale_ * gravity_ff_[i];
